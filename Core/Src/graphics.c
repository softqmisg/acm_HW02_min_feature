 ...
 */
void draw_bmp_v(unsigned char x, unsigned char y, unsigned char width ,unsigned char height,unsigned char *pic,unsigned char color)
{
	int index=0;
	for(int ypos=y;ypos<y+height;ypos+=8)
	{
		for(int xpos=x;xpos<x+width;xpos++)
		{
			index=(xpos-x)+(ypos-y)/8*width;
			glcd_pixel(xpos, ypos+0, pic[index]&0x01);
			glcd_pixel(xpos, ypos+1, pic[index]&0x02);
			glcd_pixel(xpos, ypos+2, pic[index]&0x04);
			glcd_pixel(xpos, ypos+3, pic[index]&0x08);
			glcd_pixel(xpos, ypos+4, pic[index]&0x10);
			glcd_pixel(xpos, ypos+5, pic[index]&0x20);
			glcd_pixel(xpos, ypos+6, pic[index]&0x40);
			glcd_pixel(xpos, ypos+7, pic[index]&0x80);
		}
	}
}
/*
